...
client.on("messageCreate", (message) => {
  ...
  const muaf_linkler = [    ".cdn.discordapp.com",    ".discordapp.com",    ".youtube.com"  ]

  if(linkler.some(alo => message.content.toLowerCase().includes(alo)) && 
     !muaf_linkler.some(muaf => message.content.toLowerCase().includes(muaf))) {
    message.delete()
